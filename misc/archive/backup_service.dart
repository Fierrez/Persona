...
class BackupService {
  ...
  // 1. Get Data & Encrypt it with a User Password
  ...
  static Future<bool> restoreEncryptedBackup(String encryptedContent, String password) async {
    try {
      ...
      // Decryption failed (wrong password) or invalid JSON
      return false;
    }
  }
  ...
  // PBKDF2-like behavior: Hash password to get 32-byte key
  static encrypt.Key _deriveKey(String password) {
    final bytes = utf8.encode(password);
    final digest = sha256.convert(bytes);
    return encrypt.Key(Uint8List.fromList(digest.bytes)); // 32 bytes for AES-256
  }
  ...
  static String _encryptData(String plainText, String password) {
    ...
    // Return format: IV_BASE64:ENCRYPTED_BASE64
